...
- `Calculate(sortBy string, limit int)` - вычисляет статистику с сортировкой (sortBy: "count" или "exp") и лимитом записей
...
3. Запустите `powershell test.ps1` или `build.bat`
...
- **config_test.go** - 7 тестов (загрузка, сохранение, нормализация путей, форматирование чисел)
- **parser_test.go** - 19 тестов (парсинг записей, файлов, имён, опыта)
- **stats_test.go** - 11 тестов (вычисление, форматирование, сортировка, лимит)

**Всего 37 тестов** покрывают основной функционал приложения.

### Запуск тестов с цветным выводом

```bash
# Рекомендуемый способ - красивый цветной вывод
powershell test.ps1

# Или через build.bat (включает все этапы сборки с цветными тестами)
build.bat
```
...
1. Убедитесь, что все тесты проходят: `powershell test.ps1` или `build.bat`
2. Обновите версию в комментариях (если применимо)
3. Запустите сборку: `build.bat`
4. Протестируйте выполняемый файл на реальных логах
5. Упакуйте содержимое папки `build/` в архив для распространения

---

**Вопросы?** Изучите исходный код или запустите тесты для лучшего понимания логики. Так же можно написать [мне](https://t.me/Linsaym397)
